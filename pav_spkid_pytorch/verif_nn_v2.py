...
def classify(model, fmatrix, in_frames, candidate_index):
    ...
    pred = y_.sum(dim=0)
    prob_backg = max(pred)
    prob_candidate = pred[candidate_index]
    return prob_backg,prob_candidate
...
def main(opts):
    ...
    candidates_file = open(opts.te_list_file_candidates,'r')
    candidates_list = [l.rstrip() for l in candidates_file]
    with open(opts.te_list_file, 'r') as test_f:
        ...
        for test_i, test_file in enumerate(test_list, start=1):
            ...
            candidate = candidates_list[test_i-1]
            candidate_index = list(spk2idx.keys()).index(candidate)
            prob_backg, prob_candidate = classify(model, fmatrix, cfg['in_frames'], candidate_index)
            prob_backg = prob_backg.item()/1000
            prob_candidate = prob_candidate.item()/1000
            dif = prob_candidate - prob_backg
            out_log.write('{}\t{}\t{}\t{}\t{}\n'.format(test_file, candidate, dif, prob_candidate, prob_backg))
            print('{}\t{}\t{}\t{}\t{}\n'.format(test_file, candidate, dif, prob_candidate, prob_backg))
            if opts.verbose:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--te_list_file_candidates', type=str, default='spk_rec_candidates.test',
                        help='list with names of candidates to verify (default. lists/verif/all.test.candidates)')
